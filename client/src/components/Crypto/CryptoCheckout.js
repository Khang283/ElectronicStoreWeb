...
export function CryptoCheckout(props) {
  ...
  useEffect(() => {
    axios
      .get(
        `https://v6.exchangerate-api.com/v6/${process.env.REACT_APP_EXCHANGE_RATE_API}/latest/VND`,
      )
      .then((res) => {
        setTotalPrice(
          Math.ceil(
            location.state.cart.totalPrice *
              res.data.conversion_rates.USD *
              Math.pow(10, 18),
          ),
        );
        console.log(totalPrice);
  ...
  async function transfer() {
    const data = await writeContract({
      ...
      args: [process.env.REACT_APP_WALLET_ADDRESS, totalPrice],
    });
  }
  ...
  useEffect(() => {
    if (isCheckout) {
      ...
      axios
        .post("/api/cart/checkout/confirm", payload, {
          headers: {
  ...
  if (isMetaMaskInstalled) {
    if (isConnected) {
      return (
        <div className="text-center">
          ...
          <Button disabled={isCheckout} onClick={transfer}>
            Transfer
          </Button>
    ...
    return (
      <div>
        <h2>Your browser does not have MetaMask extension installed</h2>
        <Link to={"/"}>
          <Button>Quay về trang chủ</Button>
        </Link>
      </div>
